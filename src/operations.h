...
check_result sv_backup(const sv_app *app,
    ...
    svdb_db *db,
    void *test_context);
check_result sv_compact(const sv_app *app,
    const sv_group *grp,
    svdb_db *db);
...
check_result hook_get_file_info(void *phook,
...
check_result sv_strip_archive_removing_old_files(sv_compact_state *op,
    unused_ptr(const sv_app),
    unused_ptr(const sv_group),
    svdb_db *db,
    bstrlist *messages);
check_result sv_remove_entire_archives(sv_compact_state *op,
    svdb_db *db,
...
check_result sv_compact_impl(const sv_group *grp,
    ...
    svdb_db *db,
    sv_compact_state *op);
check_result sv_compact_ask_user(const sv_group *grp,
    sv_compact_state *op);
...
check_result sv_backup_addtoqueue_cb(void *context,
    ...
    uint64_t lmt_from_disk,
    uint64_t actual_size_from_disk,
    const bstring permissions);
