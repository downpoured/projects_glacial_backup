...
check_result roughselectstrfromjson(const char *json, const char *key, bstring result) {
    ...
    check_b(!strstr(fnd + 1, cstr(search)), "key found more than once, %s in %s", key, json);
    bstrlist_splitcstr(partsAfterQuote, fnd + blength(search), '"');
    check_b(partsAfterQuote->qty > 1, "closing quote not found %s in %s", key, json);
...
bstring localpath_to_cloud_path(const char *crootdir, const char *cpath, const char *groupname) {
    /* normalize pathsep */
    ...
    check_fatal(cstr(rootdir)[blength(rootdir) - 1] != '/', "root should not end with dirsep");
    bcatcstr(rootdir, "/");
    ...
    *isdirty = true;
    ...
    if (cloud_size == 0 && cloud_crc32 == 0 && cloud_modtime == 0) {
        ...
        *isdirty = true;
        ...
        /* case 2: optimization: skip computing crc if modtime and filesize match, assume file is the same */
        *isdirty = false;
    }
    else {
        uint32_t intcrc = 0;
...
check_result sv_sync_finddirtyfiles_cb(void *context,
    ...
    bstring cloudpath = localpath_to_cloud_path(cstr(self->rootdir), cstr(filepath), cstr(self->groupname));
...
check_result sv_sync_findcheckawscli(bstring awscli) {
    ...
    if (!blength(awscli)) {
        ...
        const char *args[] = { linuxonly(cstr(awscli)) "--version",
        NULL };
        int retcode = -1;
        setdirforcli(cstr(awscli));
        check(os_run_process(cstr(awscli), args, output, temp, false, NULL, NULL, &retcode));
        bltrimws(output);
...
check_result sv_sync_rawupload(const char *awscli, const char *vaultname, const char *path, const char *description, bstring outarchiveid) {
    ...
    const char *args[] = { linuxonly(awscli) "glacier",
        ...
        "--body",
        path, NULL };
    log_b(false, "starting a typical upload, vaultname=%s description=%s path=%s", vaultname, description, path);
    bstring output = bstring_open();
    bstring temp = bstring_open();
    int retcode = -1;
    setdirforcli(awscli);
...
check_result sv_sync_marksuccessupload(svdb_db *db, const char *filepath, const char *cloudpath, const char *description, const char *archiveid, uint64_t sz, uint64_t knownvaultid) {
    ...
    check(svdb_vaultarchives_insert(db, cloudpath, description, knownvaultid, archiveid, sz, intcrc, modtime));
cleanup:
    return currenterr;
}

check_result sv_sync_uploadone(svdb_db *db, const char *awscli, int i, int total, const char *root, const char *grpname, const char *sizeandfile, const char *vault, uint64_t knownvaultid)
{
    sv_result currenterr = {};
    ...
    if (os_file_exists(filepath)) {
        ...
        check_b(blength(cloudpath), "could not make cloudpath %s, %s", root, filepath);
        ...
        do {
            ...
            sv_result err = sv_sync_rawupload(awscli, vault, filepath, cstr(description), archiveid);
            if (err.code) {
        ...
        if (succeeded) {
            check(sv_sync_marksuccessupload(db, filepath, cstr(cloudpath), cstr(description), cstr(archiveid), sz, knownvaultid));
        }
    } else {
...
check_result sv_sync_main(const sv_app *app, const sv_group *grp, svdb_db *db, const char *region, const char *vault, uint64_t knownvaultid)
{
    ...
    printf("%d file(s) need to be uploaded (%f Mb)\n\n", finder.sizes_and_files->qty, (double)finder.totalsizedirty / (1024.0 * 1024.0));
    if (ask_user("Upload the files now?")) {
        for (int i=0; i<finder.sizes_and_files->qty; i++) {
            check(sv_sync_uploadone(db, cstr(awscli), i, finder.sizes_and_files->qty, cstr(finder.rootdir),
                cstr(finder.groupname),
    ...
    const sv_app *app, const sv_group *grp, svdb_db *db)
{
    ...
    int chosen = menu_choose(
        "Please choose a Amazon Glacier vault.", names, NULL, "Create...", "(Back)");
    if (chosen == names->qty)
    {
        /* create a new vault */
        ...
        check(sv_sync_main(app, grp, db, blist_view(regions, chosen),
            blist_view(awsnames, chosen), sv_array_at64u(&ids, cast32s32u(chosen))));
    }
