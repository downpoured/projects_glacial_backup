...
check_result sv_grp_load(svdb_db *db,
    sv_group *self,
    const char *grpname);
check_result sv_grp_persist(svdb_db *db,
    const sv_group *self);
...
check_result sv_app_creategroup_impl(sv_app *self,
    ...
    const char *groupdir);
void sv_app_groupdbpathfromname(const sv_app *app,
    const char *grpname,
    bstring outpath);
...
check_result sv_verify_archives(const sv_app *app,
    ...
    svdb_db *db,
    int *countmismatches);
check_result write_archive_checksum(svdb_db *db,
    const char *filepath,
