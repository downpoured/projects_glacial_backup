...
SV_BEGIN_TEST_SUITE(tests_sync_cloud_standalone)
{
    ...
    {
        expect_err_with_message(roughselectstrfromjson(cstr(json), "NotFound", result), "key not found");
        expect_err_with_message(roughselectstrfromjson(cstr(json), "Duplicate", result), "more than once");
    ...
    {
        TEST_OPEN_EX(bstring, s1, localpath_to_cloud_path("C:\\dir1\\dir2", "C:\\dir1\\dir2\\file.dat", "a"));
        TestEqs("glacial_backup/a/dir2/file.dat", cstr(s1));
        TEST_OPEN_EX(bstring, s2, localpath_to_cloud_path("C:\\dir1\\dir2", "C:\\dir1\\dir2\\dir3\\file.dat", "a"));
        TestEqs("glacial_backup/a/dir2/dir3/file.dat", cstr(s2));
        TEST_OPEN_EX(bstring, s3, localpath_to_cloud_path("C:\\dir1\\dir2", "C:\\dir1\\dir2\\dir3\\dir4\\file.dat", "a"));
        TestEqs("glacial_backup/a/dir2/dir3/dir4/file.dat", cstr(s3));
        TEST_OPEN_EX(bstring, s4, localpath_to_cloud_path("C:\\dir1\\dir2", "C:\\dir1\\dir2file.dat", "a"));
        TestEqs("", cstr(s4));
        TEST_OPEN_EX(bstring, s5, localpath_to_cloud_path("C:\\dir1\\dir2", "C:\\dir1\\file.dat", "a"));
        TestEqs("", cstr(s5));
        TEST_OPEN_EX(bstring, s6, localpath_to_cloud_path("C:\\dir1\\dir2", "file.dat", "a"));
        TestEqs("", cstr(s6));
    }

    SV_TEST("localpath_to_cloud_path unix-style")
    {
        TEST_OPEN_EX(bstring, s1, localpath_to_cloud_path("/dir1/dir2", "/dir1/dir2/file.dat", "a"));
        TestEqs("glacial_backup/a/dir2/file.dat", cstr(s1));
        TEST_OPEN_EX(bstring, s2, localpath_to_cloud_path("/dir1/dir2", "/dir1/dir2/dir3/file.dat", "a"));
        TestEqs("glacial_backup/a/dir2/dir3/file.dat", cstr(s2));
        TEST_OPEN_EX(bstring, s3, localpath_to_cloud_path("/dir1/dir2", "/dir1/dir2/dir3/dir4/file.dat", "a"));
        TestEqs("glacial_backup/a/dir2/dir3/dir4/file.dat", cstr(s3));
        TEST_OPEN_EX(bstring, s4, localpath_to_cloud_path("/dir1/dir2", "/dir1/dir2file.dat", "a"));
        TestEqs("", cstr(s4));
        TEST_OPEN_EX(bstring, s5, localpath_to_cloud_path("/dir1/dir2", "/dir1/file.dat", "a"));
        TestEqs("", cstr(s5));
        TEST_OPEN_EX(bstring, s6, localpath_to_cloud_path("/dir1/dir2", "file.dat", "a"));
        TestEqs("", cstr(s6));
    }
    ...
    SV_TEST("cloud_path_to_description")
    {
        ...
        TEST_OPEN_EX(bstring, desc, cloud_path_to_description("a/example001.txt", cstr(tmppath)));
        ...
        const char *expectlike = "<m><v>4</v><p>YTtleGFtcGxlMDAxLnR4dA==</p><lm>20200815T172139Z</lm></m>";
        TestTrue(s_startwith(cstr(desc), "<m><v>4</v><p>YTtleGFtcGxlMDAxLnR4dA==</p><lm>2"));
        TestTrue(s_endwith(cstr(desc), "Z</lm></m>"));
        TestEqn(strlen(cstr(desc)), strlen(expectlike));
        printf("\nWe got \n%s\n, which aside from the date+time should look similar \n%s\n.", cstr(desc), expectlike);
    }

    SV_TEST("base64")
    {
        const char *cb64 = "ZHVwbGljYXRpL2RhdGFfMV9wb3N0cy9kdXBsaWNhdGktYjcxYmZiMDRlY2Y3MTQzZGM5NTQ5ZGMxNWQ0YmZmNWI5LmRibG9jay56aXAuYWVz";
        const char *cdec = "duplicati/data_1_posts/duplicati-b71bfb04ecf7143dc9549dc15d4bff5b9.dblock.zip.aes";
...
check_result test_sync_cloud_db_vaults(
    ...
    check(svdb_runsql(db, s_and_len("DELETE FROM TblKnownVaults WHERE 1"), expectchangesunknown));
    ...
    check(svdb_runsql(db, s_and_len("DELETE FROM TblKnownVaults WHERE name='nm2'"), expectchangesunknown));
    {
        TEST_OPEN_EX(bstrlist *, regions, bstrlist_open());
        TEST_OPEN_EX(bstrlist *, names, bstrlist_open());
        TEST_OPEN_EX(bstrlist *, awsnames, bstrlist_open());
...
check_result expect_vaultarchives_row(svdb_db *db, const char *path, uint64_t knownvaultid, uint64_t size, uint64_t crc32, uint64_t modtime) {
    ...
    sv_result ret = svdb_vaultarchives_bypath(db, path, knownvaultid, &outsize, &outcrc32, &outmodtime);
    if (ret.code)
    {
        return ret;
    }
...
check_result test_sync_cloud_db_vaultarchives(
    svdb_db *db) {
    sv_result currenterr = {};
    check(svdb_runsql(db, s_and_len("DELETE FROM TblKnownVaultArchives WHERE 1"), expectchangesunknown));

    /* disallow duplicate paths */
    check(svdb_vaultarchives_insert(db, "path1", "descpath1", 11, "awsid1", 1002, 1003, 1004));
    expect_err_with_message(svdb_vaultarchives_insert(db, "path1", "descpath1diff", 11, "awsid1diff", 2002, 2003, 2004), "");

    /* insert rows */
    check(svdb_vaultarchives_insert(db, "path1", "descpath1", 22, "awsid2", 10002, 10003, 10004));
    check(svdb_vaultarchives_insert(db, "path2", "descpath2", 22, "awsid2", 20002, 20003, 20004));
    check(svdb_vaultarchives_insert(db, "path3", "descpath3", 22, "awsid3", 30002, 30003, 30004));
    check(svdb_vaultarchives_insert(db, "path4", "descpath4", 22, "awsid4", 40002, 40003, 40004));
    ...
    check(svdb_vaultarchives_insert(db, "path1", "descpath1", 22, "awsid2", 90002, 90003, 90004));
    check(expect_vaultarchives_row(db, "path1", 11, 1002, 1003, 1004));
    ...
    check(svdb_vaultarchives_insert(db, "path2", "descpath2", 22, "awsid2", 990002, 990003, 990004));
    check(expect_vaultarchives_row(db, "path1", 22, 90002, 90003, 90004));
...
check_result test_sync_cloud_finddirty(svdb_db *db, const char *tmpdir) {
    ...
    check(sv_sync_marksuccessupload(db, cstr(tmppath), "glacial_backup/a/untar/f2.txt", "desc2", "id2", 4, v));
    check(tmpwritetextfile(tmpdir, "f3.txt", tmppath, "abcde"));
    check(sv_sync_marksuccessupload(db, cstr(tmppath), "glacial_backup/a/untar/f3.txt", "desc3", "id3", 5, v));
    check(tmpwritetextfile(tmpdir, "f4.txt", tmppath, "abcdef"));
    check(sv_sync_marksuccessupload(db, cstr(tmppath), "glacial_backup/a/untar/f4.txt", "desc4", "id4", 6, v));
    check(tmpwritetextfile(tmpdir, "f5.txt", tmppath, "abcdefg"));
    check(sv_sync_marksuccessupload(db, cstr(tmppath), "glacial_backup/a/untar/f5.txt", "desc5", "id5", 7, v));
    check(tmpwritetextfile(tmpdir, "f6.txt", tmppath, "abcdefgh"));
    check(sv_sync_marksuccessupload(db, cstr(tmppath), "glacial_backup/a/untar/f6.txt", "desc6", "id6", 8, v));
    check(tmpwritetextfile(tmpdir, "f7.txt", tmppath, "abcdefghi"));
    check(sv_sync_marksuccessupload(db, cstr(tmppath), "glacial_backup/a/untar/f7.txt", "desc7", "id7", 9, v));
    check(tmpwritetextfile(tmpdir, "f8.txt", tmppath, "abcdefghij"));
    check(sv_sync_marksuccessupload(db, cstr(tmppath), "glacial_backup/a/untar/f8.txt", "desc8", "id7", 10, v));
    check(tmpwritetextfile(tmpdir, "f9.txt", tmppath, "abcdefghijk"));
    check(sv_sync_marksuccessupload(db, cstr(tmppath), "glacial_backup/a/untar/f9.txt", "desc9", "id2", 11, v));
    ...
    check(sv_sync_marksuccessupload(db, cstr(tmppath), "f1.txt", "desc1", "id1", 3, v + 1));
    ...
    bsetfmt(sql, "UPDATE TblKnownVaultArchives SET ModifiedTime='%llu' WHERE CloudPath='glacial_backup/a/untar/f3.txt'", modtime);
    ...
    bsetfmt(sql, "UPDATE TblKnownVaultArchives SET Crc32='%u' WHERE CloudPath='glacial_backup/a/untar/f4.txt'", intcrc);
    check(svdb_runsql(db, cstr(sql), blength(sql), expectchanges));
    ...
    bsetfmt(sql, "UPDATE TblKnownVaultArchives SET ModifiedTime='%llu' WHERE CloudPath='glacial_backup/a/untar/f5.txt'", modtime);
    check(svdb_runsql(db, cstr(sql), blength(sql), expectchanges));
    ...
    check(sv_basic_crc32_wholefile(cstr(tmppath), &intcrc));
    bsetfmt(sql, "UPDATE TblKnownVaultArchives SET Crc32='%u' WHERE CloudPath='glacial_backup/a/untar/f8.txt'", intcrc);
    check(svdb_runsql(db, cstr(sql), blength(sql), expectchanges));
    ...
    TEST_OPEN_EX(bstring, allclean , bjoin_static(finder.sizes_and_files_clean, "|"));
    ...
    /* check results */
    TestEqs("00000003|root/f1.txt|00000008|root/f5.txt|00000009|root/f7.txt|0000000c|root/f9.txt", cstr(alldirty));
    TestEqs("00000004|root/f2.txt|00000005|root/f3.txt|00000007|root/f4.txt|00000008|root/f6.txt|0000000b|root/f8.txt", cstr(allclean));
cleanup:
    return currenterr;
}
