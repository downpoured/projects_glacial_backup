...
check_result create_test_tar(const char *tar,
    const char *tempdir,
    ar_util *ar,
    int nfiles);
check_result create_test_xz(const char *xz,
    const char *tempdir,
    ar_util *ar,
    bool large);
...
SV_BEGIN_TEST_SUITE(tests_tar)
{
    ...
    SV_TEST_WIN("attempt to add file larger than max size") {
        ...
        expect_err_with_message(ar_util_add(&ar, cstr(tar), cstr(path),
            "namewithin.txt", 1024ULL * 1024 * 1024), "files of this size");
    }

    SV_TEST("attempt adding missing file to new tar") {
        TEST_OPEN_EX(bstring, tar, bformat(
            "%s%s%s.tar", tempdir, pathsep, currentcontext));
        TEST_OPEN_EX(bstring, path, bformat(
            "%s%snot-exist.txt", tempdir, pathsep));
        TEST_OPEN(ar_util, ar);
        check(checkbinarypaths(&ar, false, tempdir));
        expect_err_with_message(ar_util_add(&ar, cstr(tar), cstr(path),
            "namewithin", 0), "Cannot stat: No such file");
    }
    ...
    SV_TEST("attempt adding missing file to existing tar") {
        ...
        expect_err_with_message(ar_util_add(
            &ar, cstr(tar), cstr(path), "namewithin", 0),
    ...
    SV_TEST("attempt to use missing tar") {
        TEST_OPEN_EX(bstring, tempsubdir, bformat("%s%ssub", tempdir, pathsep));
        TEST_OPEN_EX(bstring, tar, bformat(
            "%s%sdoes-not-exist--.tar", tempdir, pathsep));
        TEST_OPEN_EX(bstrlist *, list, bstrlist_open());
        TEST_OPEN_EX(sv_array, sizes, sv_array_open_u64());
        TEST_OPEN(ar_util, ar);
        sv_array_add64u(&sizes, 0x1);
        check(checkbinarypaths(&ar, false, tempdir));
        expect_err_with_message(tests_tar_list(&ar, cstr(tar), list),
            "Cannot open: No such");
        expect_err_with_message(ar_util_verify(&ar, cstr(tar), list, &sizes),
            "Cannot open: No such");
        expect_err_with_message(ar_util_delete(&ar, cstr(tar), tempdir,
            cstr(tempsubdir), &sizes), "Cannot open: No such");
    }
    ...
    SV_TEST("validate tar containing 1 file") {
        ...
        check(ar_util_add(&ar, cstr(tar), cstr(path),
            "0000007b.file", os_getfilesize(cstr(path))));
        check(tests_tar_list(&ar, cstr(tar), list));
    ...
    SV_TEST("validation should fail if too many filenames given") {
        ...
        bstrlist_splitcstr(list,
            "0000007b.file|000001c8.file|000001c9.file", '|');
        expect_err_with_message(ar_util_verify(&ar, cstr(tar), list, &arrsizes),
    ...
    SV_TEST("extract first file") {
        ...
        TEST_OPEN_EX(bstring, tempsubdir, bformat("%s%ssub", tempdir, pathsep));
        TEST_OPEN_EX(bstring, path, bformat(
            "%s%s0000007b.file", cstr(tempsubdir), pathsep));
        TEST_OPEN2(bstring, restored_to, contents);
        TEST_OPEN(ar_util, ar);
        check(create_test_tar(cstr(tar), tempdir, &ar, 2));
        check(tests_cleardir(cstr(tempsubdir)));
        check(ar_util_extract_overwrite(&ar, cstr(tar),
            "0000007b.file", cstr(tempsubdir), restored_to));
    ...
    SV_TEST("extract second file") {
        ...
        TEST_OPEN_EX(bstring, tempsubdir, bformat("%s%ssub", tempdir, pathsep));
        TEST_OPEN_EX(bstring, path, bformat(
            "%s%s000001c8.file", cstr(tempsubdir), pathsep));
        TEST_OPEN2(bstring, restored_to, contents);
        TEST_OPEN(ar_util, ar);
        check(create_test_tar(cstr(tar), tempdir, &ar, 2));
        check(tests_cleardir(cstr(tempsubdir)));
        ...
        check(ar_util_extract_overwrite(&ar, cstr(tar),
            "000001c8.file", cstr(tempsubdir), restored_to));
        check(sv_file_readfile(cstr(restored_to), contents));
    ...
    SV_TEST("extracting from missing tar should fail") {
        TEST_OPEN_EX(bstring, tar, bformat(
            "%s%snotexist.tar", tempdir, pathsep));
        TEST_OPEN(ar_util, ar);
        TEST_OPEN(bstring, restored_to);
        check(checkbinarypaths(&ar, false, tempdir));
        expect_err_with_message(ar_util_extract_overwrite(
            &ar, cstr(tar), "*", tempdir, restored_to), "Cannot open: No such");
    }

    SV_TEST("extracting missing file from tar should fail") {
        TEST_OPEN_EX(bstring, tar, bformat(
            "%s%s%s.tar", tempdir, pathsep, currentcontext));
        TEST_OPEN_EX(bstring, tmp, bformat(
            "%s%ssub", tempdir, pathsep));
        TEST_OPEN(ar_util, ar);
        TEST_OPEN(bstring, restored_to);
        check(create_test_tar(cstr(tar), tempdir, &ar, 2));
        check(tests_cleardir(cstr(tmp)));
        expect_err_with_message(ar_util_extract_overwrite(&ar, cstr(tar),
            "0000007b.xz", cstr(tmp), restored_to), "Not found in archive");
    }
    ...
    SV_TEST("cannot add to archive if dest locked") {
        ...
        TEST_OPEN_EX(bstring, tar, bformat(
            "%s%s%s.tar", tempdir, pathsep, currentcontext));
        TEST_OPEN_EX(bstring, path, bformat(
            "%s%s00000315.file", tempdir, pathsep));
        TEST_OPEN(ar_util, ar);
        check(create_test_tar(cstr(tar), tempdir, &ar, 2));
        check(tests_lockfile(true, cstr(tar), &handle));
        expect_err_with_message(ar_util_add(&ar, cstr(tar), cstr(path),
            "namewithin", os_getfilesize(cstr(path))), "Cannot read:");
    ...
    SV_TEST("cannot extract from archive if src locked") {
        ...
        TEST_OPEN_EX(bstring, tar, bformat(
            "%s%s%s.tar", tempdir, pathsep, currentcontext));
        TEST_OPEN_EX(bstring, tempsubdir, bformat(
            "%s%ssub", tempdir, pathsep));
        TEST_OPEN(ar_util, ar);
        check(create_test_tar(cstr(tar), tempdir, &ar, 2));
        check(tests_cleardir(cstr(tempsubdir)));
        check(tests_lockfile(true, cstr(tar), &handle));
        expect_err_with_message(ar_util_extract_overwrite(&ar, cstr(tar),
            "0000007b.file", cstr(tempsubdir), NULL), "Cannot open:");
        check(tests_lockfile(false, cstr(tar), &handle));
    }
    ...
    SV_TEST_WIN("cannot extract from archive if dest locked") {
        ...
        check(tests_lockfile(true, cstr(path), &handle));
        expect_err_with_message(ar_util_extract_overwrite(&ar, cstr(tar),
            "0000007b.file", cstr(tempsubdir), NULL), "couldn't remove ");
        check(tests_lockfile(false, cstr(path), &handle));
    }
    ...
    SV_TEST("cannot validate if archive locked") {
        ...
        check(create_test_tar(cstr(tar), tempdir, &ar, 2));
        check(tests_lockfile(true, cstr(tar), &handle));
        expect_err_with_message(ar_util_verify(
            &ar, cstr(tar), list, &arrsizes), "Cannot open:");
        expect_err_with_message(tests_tar_list(
            &ar, cstr(tar), list), "Cannot open:");
        check(tests_lockfile(false, cstr(tar), &handle));
    }
}
SV_END_TEST_SUITE()
...
SV_BEGIN_TEST_SUITE(tests_xz)
{
    SV_TEST("create and validate xz file from text") {
        TEST_OPEN_EX(bstring, xz, bformat(
            "%s%s%s.xz", tempdir, pathsep, currentcontext));
        TEST_OPEN_EX(bstring, path, bformat(
            "%s%sa.txt", tempdir, pathsep));
        TEST_OPEN(ar_util, ar);
        check(checkbinarypaths(&ar, false, tempdir));
    ...
    SV_TEST("extract from xz file overwrite existing file") {
        TEST_OPEN_EX(bstring, xz, bformat(
            "%s%s%s.xz", tempdir, pathsep, currentcontext));
        TEST_OPEN_EX(bstring, path, bformat(
            "%s%s%s.txt", tempdir, pathsep, currentcontext));
    ...
    SV_TEST("validate should fail on locked xz file") {
        ...
        expect_err_with_message(ar_util_xz_verify(&ar,
            cstr(xz)), "Permission denied");
    ...
    SV_TEST("create should fail if src is missing") {
        ...
        TEST_OPEN(ar_util, ar);
        check(checkbinarypaths(&ar, false, tempdir));
        expect_err_with_message(ar_util_xz_add(&ar, cstr(path),
            cstr(xz)), islinux ? "see short path" : "get short path");
    }

    SV_TEST("create should fail if src is locked") {
        TEST_OPEN_EX(bstring, xz, bformat("%s%s%s.xz",
            tempdir, pathsep, currentcontext));
        TEST_OPEN_EX(bstring, path, bformat("%s%sa.txt", tempdir, pathsep));
        TEST_OPEN_EX(os_lockedfilehandle, handle, {});
        TEST_OPEN(ar_util, ar);
        check(checkbinarypaths(&ar, false, tempdir));
        check(sv_file_writefile(cstr(path), "abcdef", "wb"));
        check(tests_lockfile(true, cstr(path), &handle));
        expect_err_with_message(ar_util_xz_add(
            &ar, cstr(path), cstr(xz)), "not be 0 bytes");
        check(tests_lockfile(false, cstr(path), &handle));
    }
    ...
    SV_TEST("create should fail if dest is locked") {
        ...
        expect_err_with_message(ar_util_xz_add(
            &ar, cstr(path), cstr(xz)), "Permission denied(13)");
        check(tests_lockfile(false, cstr(xz), &handle));
    }
    ...
    SV_TEST("extract should fail if dest is locked") {
        TEST_OPEN_EX(bstring, xz, bformat("%s%s%s.xz",
            tempdir, pathsep, currentcontext));
        TEST_OPEN_EX(bstring, path, bformat("%s%sa.txt",
            tempdir, pathsep));
        TEST_OPEN_EX(os_lockedfilehandle, handle, {});
        TEST_OPEN(ar_util, ar);
        check(create_test_xz(cstr(xz), tempdir, &ar, false));
        ...
        check(tests_lockfile(true, cstr(path), &handle));
        expect_err_with_message(ar_util_xz_extract_overwrite(&ar,
            cstr(xz), cstr(path)), "Permission denied(13)");
        check(tests_lockfile(false, cstr(path), &handle));
    }
    ...
    SV_TEST("extract should fail if archive is missing") {
        ...
        TEST_OPEN(ar_util, ar);
        check(checkbinarypaths(&ar, false, tempdir));
        expect_err_with_message(ar_util_xz_extract_overwrite(&ar, cstr(xz),
            cstr(path)), islinux ? "see short path" : "get short path");
    }
}
SV_END_TEST_SUITE()
...
SV_BEGIN_TEST_SUITE(whole_tests_archive_filenames)
{
    ...
    TEST_OPEN_EX(bstrlist *, list, bstrlist_open());
    TEST_OPEN_EX(sv_array, arrsizes, sv_array_open_u64());
    TEST_OPEN_EX(bstring, restoreto, bformat(
        "%s%srestoreto", dir, pathsep));
    TEST_OPEN_EX(bstring, file_normal, bformat(
        "%s%snormal.txt", dir, pathsep));
    TEST_OPEN_EX(bstring, file_large, bformat(
        "%s%slarge.txt", dir, pathsep));
    TEST_OPEN_EX(bstring, file_empty, bformat(
        "%s%s.empty.xz", dir, pathsep));
    TEST_OPEN_EX(bstring, file_utf, bformat(
        "%s%sutf_\xF0\x9D\x84\x9E.tar", dir, pathsep));
    TEST_OPEN_EX(bstring, file_name1, bformat(
        "%s%s%s", dir, pathsep, islinux ? "*.txt" : ".txt"));
    TEST_OPEN_EX(bstring, file_name2, bformat("%s%s%s",
        dir, pathsep, islinux ? ".\"\n::'a'\\n\\a[ab]?.txt" : ".'a'[ab].txt"));
    TEST_OPEN_EX(bstring, file_name3, bformat("%s%s%s", dir,
        pathsep, "-beginswithdash and has spaces"));
    ...
    bstring inputs[] = { file_normal, file_large, file_empty,
        file_utf, file_name1, file_name2, file_name3 };
    ...
    for (int i = 0; i < countof(inputs); i++)
    {
        ...
        check(ar_util_add(&ar, cstr(archive), cstr(inputs[i]),
            "namewithin.txt", os_getfilesize(cstr(inputs[i]))));
        TestTrue(os_getfilesize(cstr(archive)) >
        ...
        TestEqs(inputs[i] == file_large ? cstr(large) :
            inputs[i] == file_empty ? "" :
            cstr(inputs[i]), cstr(contents_got));
    ...
    for (int i = 0; i < countof(inputs); i++)
    {
        ...
        check(ar_util_xz_extract_overwrite(
            &ar, cstr(archive), cstr(expectedout)));
...
check_result tests_check_tar_contents(ar_util *ar,
    const char *archive,
    const char *expected,
    bool check_order)
{
    ...
    {
        if (expect_names->qty && s_equal(blist_view(
                expect_names, expect_names->qty - 1), "filenames.txt"))
        {
...
check_result tests_tar_list(ar_util *self,
    const char *tarpath,
    bstrlist *results)
{
    sv_result currenterr = {};
    ...
    int nfiles)
{
    ...
    check(ar_util_add(ar, tar, cstr(path1), "0000007b.file",
        os_getfilesize(cstr(path1))));
    check(ar_util_add(ar, tar, cstr(path2), "000001c8.file",
        os_getfilesize(cstr(path2))));
    check_b(nfiles == 2 || nfiles == 4, "");
    if (nfiles == 4)
    {
        check(ar_util_add(ar, tar, cstr(path3), "00000315.file",
            os_getfilesize(cstr(path3))));
        check(ar_util_add(ar, tar, cstr(path4), "00000316.file",
            os_getfilesize(cstr(path4))));
    }
