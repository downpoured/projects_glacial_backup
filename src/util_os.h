...
check_result sv_file_readfile(const char* filename,
	bstring contents);
...
typedef sv_result(*FnRecurseThroughFilesCallback)(void* context,
	const bstring filepath, uint64_t modtime, uint64_t filesize, uint64_t permissions);
...
typedef struct os_recurse_params {
	...
	bstrList* symlinks_skipped;
	bstrList* dirs_not_accessible;
	int max_tries;
	int sleep_between_tries;
} os_recurse_params;
...
check_result os_run_process_ex(const char* path, const char* const args[], os_proc_op op, 
	os_exclusivefilehandle* providestdin, bool fastjoinargs, bstring useargscombined, bstring getoutput, int* retcode);
